 ...
 * @created : 22Dec2021
 *     <p>Exercise 4.3 (Geography: estimate areas) Use the GPS locations for Atlanta, Georgia;
 *     Orlando, Florida; Savannah, Georgia; and Charlotte, North Carolina in the figure in Section
 *     4.1 to compute the estimated area enclosed by these four cities. (Hint: Use the formula in
 *     Programming Exercise 4.2 to compute the distance between two cities. Divide the polygon into
 *     two triangles and use the formula in Programming Exercise 2.19 to compute the area of a
 *     triangle.)
 */
...
public class Exercise03 {
  public static void main(String[] args) {
    ...
    double charlotteToAtlanta =
        6371.01
            * Math.acos(
                Math.sin(x4) * Math.sin(x1) + Math.cos(x4) * Math.cos(x1) * Math.cos(y4 - y1));
    double atlantaToSavannah =
        6371.01
            * Math.acos(
                Math.sin(x1) * Math.sin(x3) + Math.cos(x1) * Math.cos(x3) * Math.cos(y1 - y3));
    ...
    System.out.printf(
        "The area between Atlanta, Georgia, Orlando, Florida, Savannah, Georgia, "
            + "and Charlotte, North Carolina is about %f",
        area1 + area2);
  }
}
